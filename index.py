...
JEDA_SEMUA = 5  # SEMUA JEDA 5 DETIK
...
# ✅ FUNGSI KIRIM DIPERBAIKI PASTI ADA STATUS KIRIMNYA!
def kirim_pancing(email_target):
    try:
        ...
        # Kirim request dengan timeout pas
        res = requests.post(
            "https://web.prod.cloud.netflix.com/graphql",
            headers=headers_netflix,
            cookies=cookie_pancing,
            json=payload,
            timeout=15
        )
        # Cek status kirim
        if res.status_code in [200, 201]:
            return True, "✅ BERHASIL TERKIRIM! Event 30 hari sudah dipancing"
        else:
            return False, f"❌ GAGAL KIRIM! Kode: {res.status_code} | {res.text[:50]}"
    except Exception as e:
        return False, f"❌ ERROR KIRIM: {str(e)[:60]}"
...
<head>
    ...
    <title>Netflix Event - Fix Kirim</title>
    <style>
...
<body>
    <h1>🔥 NETFLIX EVENT 30 HARI - PASTI KIRIM 🔥</h1>

    <div id="running" class="running">⏳ SEDANG DIJALANKAN... JANGAN DITUTUP!</div>

    <div class="menu">
        <h3>📋 PILIHAN:</h3>
        <div class="pilihan" onclick="tampilForm(1)">
            <b>1. OTOMATIS BUAT</b> - Masukkan jumlah & sandi
        </div>
    ...
    <div id="form1" class="form">
        ...
        <form method="POST" onsubmit="tampilRun()">
            ...
            <input type="number" name="jumlah" placeholder="JUMLAH AKUN" min="1" max="10" required>
            <input type="text" name="sandi" placeholder="SANDI SEMUA AKUN" required>
            <button type="submit">🚀 MULAI</button>
        </form>
    </div>

    <div id="form2" class="form">
        <h3>⚙️ MANUAL KIRIM</h3>
        <form method="POST" onsubmit="tampilRun()">
            <input type="hidden" name="tipe" value="2">
            <input type="number" name="jumlah" id="jml_email" placeholder="JUMLAH EMAIL" min="1" max="10" required oninput="buatKolomEmail(this.value)">
    ...
    <div class="input-bebas">
        <h3>✏️ CATATAN:</h3>
        <textarea placeholder="Simpan link atau catatan disini..."></textarea>
    </div>
...
def utama():
    ...
    if request.method == 'POST':
        ...
        if tipe == '1':
            ...
            hasil_teks += f"✅ === PROSES OTOMATIS {jumlah} AKUN ===\n\n"
            ...
            for no in range(1, jumlah+1):
                ...
                try:
                    ...
                    res_buat = requests.post(f"{BASE_MAILTM}/accounts", headers=headers_mailtm, json={"address": email_baru, "password": sandi}, timeout=15)
                    if res_buat.status_code not in [200,201]:
                        hasil_teks += f"❌ Gagal buat email: {email_baru}\n\n"
                        continue
                    hasil_teks += f"✅ Berhasil buat: {email_baru}\n"

                    # ✅ KIRIM PANCING & TAMPILKAN STATUS JELAS
                    ok, pesan_kirim = kirim_pancing(email_baru)
                    hasil_teks += f"📤 {pesan_kirim}\n"
                    time.sleep(JEDA_SEMUA)

                    # Cek pesan
                    login_res = requests.post(f"{BASE_MAILTM}/token", headers=headers_mailtm, json={"address": email_baru, "password": sandi}, timeout=10)
                    token = login_res.json()["token"]
                    auth_header = {**headers_mailtm, "Authorization": f"Bearer {token}"}
                    pesan_netflix = None
                    for _ in range(4):
                        pesan_res = requests.get(f"{BASE_MAILTM}/messages?page=1", headers=auth_header, timeout=10)
                        data = pesan_res.json()
                    ...
                    if pesan_netflix:
                        isi = requests.get(f"{BASE_MAILTM}/messages/{pesan_netflix['id']}", headers=auth_header, timeout=10).json()
                        teks = isi.get("text", "") or isi.get("html", "")
                        link_kotor = re.search(r"https?://[^\s\)\<\>\"']+netflix\.com/epr\?code=[^\s\)\<\>\"']+", teks).group(0)
                        hasil_teks += f"🔗 Link: {bersihkan_link(link_kotor)}\n"
                    else:
                        hasil_teks += "ℹ️ Pesan belum masuk, cek nanti di kotak masuk email\n"
                    hasil_teks += "\n"

                except Exception as e:
                    hasil_teks += f"❌ Error: {str(e)[:50]}\n\n"

        # PILIHAN 2: MANUAL
        if tipe == '2':
            jumlah = int(request.form.get('jumlah', 1))
            hasil_teks += f"✅ === PROSES MANUAL {jumlah} EMAIL ===\n\n"

            for no in range(1, jumlah+1):
                email = request.form.get(f'email_{no}', '').strip()
                if not email:
                    hasil_teks += f"--- EMAIL KE-{no}: KOSONG ---\n\n"
                    continue
                hasil_teks += f"--- EMAIL KE-{no}: {email} ---\n"
                # ✅ KIRIM & TAMPILKAN HASIL LANGSUNG
                ok, pesan_kirim = kirim_pancing(email)
                hasil_teks += f"📤 {pesan_kirim}\n\n"
                time.sleep(JEDA_SEMUA)
