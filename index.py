...
# 👉 GANTI DENGAN API KEY LU NANTI JIKA PUNYA!
API_KEY_2CAPTCHA = "ISI_API_KEY_LU_DI_SINI_NANTI"
...
# ======================
# 🤖 AMBIL TOKEN OTOMATIS (SIAP PAKAI KUNCI LU)
# ======================
def ambil_token_captcha():
    log = []
    log.append("🤖 SEDANG MEMINTA TOKEN CAPTCHA OTOMATIS...")
    
    if API_KEY_2CAPTCHA == "ISI_API_KEY_LU_DI_SINI_NANTI":
        log.append("⚠️ BELUM DIISI API KEY LAYANAN CAPTCHA!")
        log.append("👉 Ganti variabel API_KEY_2CAPTCHA dengan kunci lu nanti")
        log.append("   (Bisa pakai 2Captcha, Anti-Captcha, dll)")
    ...
    try:
        ...
        if data_buat.get("errorId") != 0:
            log.append(f"❌ Gagal: {data_buat.get('errorDescription', 'Kesalahan layanan')}")
            return None, "<br>".join(log)
        
        task_id = data_buat["taskId"]
        log.append(f"⏳ Tunggu selesai (ID: {task_id})...")
        
        # Cek hasil berkali-kali
        for _ in range(6):
            time.sleep(3)
            cek = requests.post("https://api.2captcha.com/getTaskResult", json={
                "clientKey": API_KEY_2CAPTCHA,
                "taskId": task_id
            }, timeout=10)
            data_cek = cek.json()
            ...
            if data_cek.get("status") == "ready":
                ...
                log.append(f"❌ Tugas gagal: {data_cek.get('errorDescription')}")
                return None, "<br>".join(log)
        
        log.append("⏰ Waktu tunggu habis, coba lagi nanti")
        return None, "<br>".join(log)
    
    except Exception as e:
        log.append(f"❌ Koneksi bermasalah: {str(e)}")
        return None, "<br>".join(log)
...
def baca_email_verifikasi(email, sandi):
    ...
    try:
        ...
        if login.status_code != 200:
            log.append("❌ Gagal masuk ke akun email!")
            return "<br>".join(log)
        ...
        for cek in range(10):
            time.sleep(3)
            pesan = requests.get(f"{BASE_MAILTM}/messages?page=1", headers=headers_mail, timeout=10).json()
            if pesan["hydra:totalItems"] > 0:
                id_pesan = pesan["hydra:member"][0]["id"]
                isi = requests.get(f"{BASE_MAILTM}/messages/{id_pesan}", headers=headers_mail, timeout=10).json()
                log.append("✅ PESAN DITERIMA DARI NETFLIX!")
                log.append(f"📩 Pengirim: {isi['from']['address']}")
                ...
                if link:
                    ...
                    log.append("⚠️ Link tidak ditemukan, silakan cek isi pesan manual")
                return "<br>".join(log)
        
        log.append("⏰ Pesan tidak masuk dalam batas waktu tunggu!")
        return "<br>".join(log)
    
    except Exception as e:
        log.append(f"❌ Error baca email: {str(e)}")
        return "<br>".join(log)
...
# ======================
def jalankan_semua():
    log = ["🎁 OTOMATIS PENUH: TANPA BUKA BROWSER!", "="*60]
    
    # Ambil token otomatis
    token, info_token = ambil_token_captcha()
    log.append(info_token)
    if not token:
        return "<br>".join(log)
    
    # Buat email Mail.tm
    log.append("")
    log.append("📧 MEMBUAT EMAIL TEMPORAL BARU...")
    try:
        domain = requests.get(f"{BASE_MAILTM}/domains?page=1", timeout=10).json()["hydra:member"][0]["domain"]
        nama = ''.join(random.choices(string.ascii_lowercase + string.digits, k=8))
        email_baru = f"{nama}@{domain}"
        sandi = "UjiCoba99!"
        
        requests.post(f"{BASE_MAILTM}/accounts", headers=headers_mailtm, json={"address":email_baru,"password":sandi}, timeout=15)
    ...
    try:
        ...
        if "errors" in res.json():
            log.append(f"⚠️ Respon Netflix: {res.json()['errors'][0]['message']}")
        else:
            log.append("✅ PENDAFTARAN BERHASIL DIKIRIM!")
    except Exception as e:
        log.append(f"❌ Error koneksi ke Netflix: {str(e)}")
        return "<br>".join(log)

    # Baca email otomatis
    log.append("")
    log.append(baca_email_verifikasi(email_baru, sandi))
    
    log.append("")
    log.append("="*60)
    log.append("✅ SELESAI! SEMUA PROSES BERJALAN OTOMATIS!")
    return "<br>".join(log)

# ======================
# 🌐 HALAMAN WEB OTOMATIS PENUH
# ======================
@app.route('/')
def index():
    hasil = jalankan_semua()
    html = f"""
    <html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>OTOMATIS PENUH NETFLIX</title>
    <style>
    *{{box-sizing:border-box;margin:0;padding:0;}}
    ...
    .kotak{{background:#222;padding:20px;border-radius:10px;margin-bottom:20px;text-align:center;color:#0f0;font-size:16px;}}
    .hasil{{background:#000;padding:15px;border-radius:8px;white-space:pre-wrap;word-break:break-all;font-size:14px;line-height:1.7;}}
    </style>
    </head><body>
    <h1>🎁 OTOMATIS PENUH - TANPA BUKA BROWSER!</h1>
    <div class="kotak">
    🚀 SEDANG MEMPROSES SEMUA SECARA OTOMATIS...<br>
    Tunggu sebentar ya jir!
    </div>
