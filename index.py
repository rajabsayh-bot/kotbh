...
def kirim_pancing(email_target):
    try:
        ...
        return True, "Berhasil dikirim pancingan event 30 hari!"
...
<head>
    ...
    <title>Netflix Pancing Event - Versi Vercel</title>
    <style>
        *{box-sizing:border-box; font-family:Arial, sans-serif; margin:0; padding:0;}
        body{background:#121212; color:#fff; padding:20px; max-width:800px; margin:0 auto;}
        h1{color:#e50914; text-align:center; margin-bottom:20px;}
        .menu{background:#1e1e1e; padding:20px; border-radius:10px; margin-bottom:20px;}
        .pilihan{margin:10px 0; padding:10px; border:1px solid #333; border-radius:5px; cursor:pointer;}
        .pilihan:hover{background:#333;}
        .form{background:#1e1e1e; padding:20px; border-radius:10px; margin-bottom:20px; display:none;}
        input, button, textarea{width:100%; padding:12px; margin:8px 0; border:none; border-radius:5px; font-size:16px;}
        input{background:#333; color:#fff;}
        button{background:#e50914; color:#fff; font-weight:bold; cursor:pointer;}
        button:hover{background:#b20710;}
        .hasil{background:#1e1e1e; padding:20px; border-radius:10px; margin-bottom:20px; white-space:pre-wrap; font-family:monospace; font-size:14px;}
        .input-bebas{background:#1e1e1e; padding:20px; border-radius:10px;}
        textarea{background:#333; color:#fff; resize:vertical; min-height:100px;}
    </style>
</head>
<body>
    <h1>🔥 NETFLIX PANCING EVENT 30 HARI - VERSI VERSEL 🔥</h1>

    <div class="menu">
        <h3>📋 PILIH CARA PENGGUNAAN:</h3>
        <div class="pilihan" onclick="tampilForm(1)">
            <b>1. Otomatis Buat Akun</b><br>
            Buat email baru otomatis, pilih jumlah & tentukan sandi
        </div>
        <div class="pilihan" onclick="tampilForm(2)">
            <b>2. Manual Kirim Pancingan</b><br>
            Cukup masukan email saja (tidak butuh sandi), langsung kirim event
        </div>
    </div>

    <div id="form1" class="form">
        <h3>⚙️ FORM OTOMATIS BUAT AKUN</h3>
        <form method="POST">
            <input type="hidden" name="tipe" value="1">
            <input type="number" name="jumlah" placeholder="Masukkan jumlah akun (contoh: 5)" min="1" max="20" required>
            <input type="text" name="sandi" placeholder="Masukkan sandi untuk semua akun" required>
            <button type="submit">🚀 MULAI PROSES OTOMATIS</button>
        </form>
    </div>

    <div id="form2" class="form">
        <h3>⚙️ FORM MANUAL KIRIM</h3>
        <form method="POST">
            <input type="hidden" name="tipe" value="2">
            <input type="number" name="jumlah" placeholder="Masukkan jumlah email yang mau dikirim" min="1" max="20" required>
            <input type="text" name="email" placeholder="Masukkan email (bisa banyak dipisah koma: a@mail.com, b@mail.com)" required>
            <button type="submit">📤 KIRIM PANCINGAN</button>
        </form>
    </div>
    ...
    <div class="input-bebas">
        <h3>✏️ KOLOM INPUT BEBAS DI BAWAH:</h3>
        <textarea placeholder="Ketik catatan, link, atau apapun yang lu mau disini..."></textarea>
    </div>
...
def utama():
    ...
    if request.method == 'POST':
        ...
        if tipe == '1':
            ...
            hasil_teks += f"🔑 Sandi dipakai: {sandi}\n\n"
        ...
        # ======================
        # PILIHAN 2: MANUAL KIRIM (CUMA INPUT EMAIL!)
        # ======================
        if tipe == '2':
            jumlah = int(request.form.get('jumlah', 1))
            daftar_email = [e.strip() for e in request.form.get('email', '').split(',') if e.strip()][:jumlah]
            hasil_teks += f"✅ === PROSES MANUAL KIRIM {len(daftar_email)} EMAIL ===\n"
            hasil_teks += "⚠️ TIDAK PERLU INPUT SANDI! Cukup email saja\n\n"

            for no, email in enumerate(daftar_email, 1):
